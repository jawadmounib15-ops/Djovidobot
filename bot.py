...
def get_data(sym):
    try:
        ...
        c=pd.to_numeric(df['Close'], errors='coerce').dropna(); h=df['High']; l=df['Low']; o=df['Open']
        ...
        prev_c=float(c.iloc[-2]); prev_e20=float(ema20.iloc[-2]); prev_e50=float(ema50.iloc[-2])
        max20=float(h.rolling(20).max().iloc[-2]); min20=float(l.rolling(20).min().iloc[-2])
        ...
        if not lavoro:
            if price > max20 and r>=55 and r<=62 and e20>e50 and price>s50:
                side="BUY"; lavoro="L3 BREAKOUT REGOLATO"
            elif price < min20 and r>=38 and r<=45 and e20<e50 and price<s50:
                side="SELL"; lavoro="L3 BREAKOUT REGOLATO"
        ...
        # L5 EMA CROSS
        if not lavoro:
            if prev_e20 < prev_e50 and e20 > e50 and r>50 and r<60: side="BUY"; lavoro="L5 EMA CROSS"
            elif prev_e20 > prev_e50 and e20 < e50 and r>40 and r<50: side="SELL"; lavoro="L5 EMA CROSS"

        # L6 REVERSAL
        if not lavoro:
            body = abs(float(c.iloc[-1]) - float(o.iloc[-1])); lower_wick = float(min(c.iloc[-1], o.iloc[-1]) - l.iloc[-1]); upper_wick = float(h.iloc[-1] - max(c.iloc[-1], o.iloc[-1]))
            if lower_wick > body*1.5 and r>=35 and r<=45: side="BUY"; lavoro="L6 REVERSAL"
            elif upper_wick > body*1.5 and r>=55 and r<=65: side="SELL"; lavoro="L6 REVERSAL"

        # L7 SQUEEZE
        if not lavoro:
            if bb_width < 0.004 and r>=50 and r<=56 and price>e20 and e20>e50: side="BUY"; lavoro="L7 SQUEEZE"
            elif bb_width < 0.004 and r>=44 and r<=50 and price<e20 and e20<e50: side="SELL"; lavoro="L7 SQUEEZE"
...
if __name__=="__main__": app.run(host="0.
