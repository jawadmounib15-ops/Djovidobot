...
app = Flask(__name__)
@app.route('/')
def home(): return "V15.9 LARGATO UN PELO LIVE"
@app.route('/ping')
def ping(): return "OK"
...
def is_perfect(candles):
    ...
    try:
        ...
        r=rsi(closes)
        # V15.9 LARGATO UN PELO: 38% invece di 40%
        if low/total>=0.38 and body/total<=0.48 and r<=62:
            return "BUY", f"L1 PINBAR Coda {low/total*100:.0f}% RSI {r:.0f}"
        if up/total>=0.38 and body/total<=0.48 and r>=38:
...
def is_double_bottom(candles):
    try:
        ...
        l1=bot1[1]; l2=bot2[1]
        if abs(l1-l2)/l1 > 0.005: return None
        last=candles[-2]
        close=float(last.get('close',0)) if isinstance(last,dict) else float(last[2])
        if close > l1*1.001:
...
def bot_loop():
    send_tg("✅ *V15.9 LARGATO UN PELO ONLINE*\n💎 38% Pinbar | M/W 0.5% | Engulf\n⏰ 3min - Pronto!")
    while True:
        try:
