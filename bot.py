...
print("Bot V9.8 FIX pulito avviato")
send("Bot V9.8 FIXATO - Ora non si blocca piu - Parte ora!")
        ...
        for p in PAIRS:
            try:
                ...
                if 30 <= rsi <= 42 and price > ema50 and ema50 > ema200:
                    forte = True
                    txt = f"BUY FORTE {p} RSI {rsi:.1f} 90% Prezzo {price:.5f}"
                elif 58 <= rsi <= 70 and price < ema50 and ema50 < ema200:
                    forte = True
                    txt = f"SELL FORTE {p} RSI {rsi:.1f} 90% Prezzo {price:.5f}"
                if forte:
                    send(txt)
                    print(txt)
                    ultimo = time.time()
