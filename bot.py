    ...
    try:
        def on_msg(w, msg):
            try:
                if '42' in msg:
                    j = json.loads(msg[2:])
                    d = j[1] if len(j) > 1 else {}
                    if isinstance(d, dict):
                        if 'candles' in d:
                            candles.extend(d['candles'])
                        if 'history' in d:
                            candles.extend(d['history'])
            except:
                pass
...
def process(pair, period, label):
    try:
        k = f"{pair}_{label}"
        if k in sent and time.time()-sent[k] < 90:
            return
...
def bot_loop():
    global fail_count
    send_tg("✅ *V16 TEST 15% ONLINE*\n💥 Test segnali attivi!")
    while True:
        try:
            if fail_count >= 40:
                send_tg("⏳ *Pocket lagga un attimo... attendo*")
                fail_count = 0
                time.sleep(20)
