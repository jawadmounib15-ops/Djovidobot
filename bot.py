...
app = Flask(__name__)
@app.route('/')
def home(): return "V105.1 STRETTO REALI POCKET ONLINE"
...
def get_candles(pair):
    ...
    try:
        ...
        if r.status_code == 200:
            j = r.json()
            data = j.get('candles') or j.get('data') or []
            if len(data) > 200:
...
def bot():
    ...
    send("💀 *V105.1 STRETTO 1 PELO REALI POCKET ONLINE*\nRSI 65/35 + BB 0.10 + EMA dist 0.05% | 25 coppie")
    while True:
        try:
            now=datetime.now()
